...
--Combining Total Revenue, Avg Order Value and Retention Rate into one table
WITH RepeatCustomers AS (
    SELECT o.Customer_ID
    FROM Orders o
    GROUP BY o.Customer_ID
...
SELECT 
    ...
    (SELECT AVG(Amount) FROM Order_Details) AS Average_Order_Value,
    (CAST((SELECT COUNT(*) FROM RepeatCustomers) AS FLOAT) 
     / 
     (SELECT COUNT(*) FROM Customers)) * 100 AS Retention_Rate_Percentage;
...
--Month over month Total Sales growth:
